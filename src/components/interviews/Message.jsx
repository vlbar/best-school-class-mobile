...
export default React.forwardRef(({ message, tasks, withAvatar, withTime, isCreator, onEdit, onReply, onPing }, ref) => {
  ...
  function renderItem(message) {
    return (
      <>
        <Pressable disabled={!!message.deletedAt || message.type == 'ANSWER'} onLongPress={() => setPopupShow(true)}>
          <View
            style={[
            ...
            <View style={!withAvatar && !isCreator && { marginLeft: 40 }}>
              {withTime && (
                <Text style={[styles.messageGroupTime, isCreator && { textAlign: 'right' }]}>
                  {moment(new Date(message.submittedAt)).utcOffset(3).format('HH:mm')}
                </Text>
              )}
              <View
                style={[
  ...
  return (
    <SwipeRow
      ref={ref}
      disableLeftSwipe={!!message.deletedAt || message.type == 'ANSWER'}
      disableRightSwipe
